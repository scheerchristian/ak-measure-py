...
akmeasure.io_setup), config/settings.yaml (measurement settings) and
...
def main():
    ...
    if reuse_session:
        ...
        if ref_cfg["type"] == "complex":
            ...
            latency_time = deconvolve(reference_signal, sweep, freq_range, regu_within).time[0]
        else:
    ...
    if reuse_session:
        ...
        if reference_signal is not None and ref_cfg["type"] == "complex":
            tf = deconvolve(reference_signal, sweep, freq_range, regu_within)
    ...
    for group in channel_groups:
        ...
        if ref_cfg["type"] == "complex":
            ...
            ir = deconvolve(recorded, sweep, freq_range, regu_within)
            if ref_cfg["type"] == "latency":
